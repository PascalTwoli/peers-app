...
export default function VideoInterface() {
	...
	return (
		<div
			...
			<div
				...
				<div className="flex items-center gap-2 md:gap-3">
					...
					<div className="flex flex-col items-center">
						<span className="text-[10px] text-gray-400 uppercase tracking-wider hidden md:block">
							Connected
						</span>
						<span className="font-semibold tabular-nums text-base md:text-lg">
			...
			<div className="absolute top-16 md:top-20 right-3 md:right-5 z-20 bg-black/60 border border-white/10 rounded-xl px-3 py-2 text-[11px] md:text-xs backdrop-blur-sm">
				<div>Bitrate: {callStats?.bitrateKbps || 0} kbps</div>
				<div>Latency: {callStats?.latencyMs || 0} ms</div>
				<div>Packet Loss: {callStats?.packetLossPct || 0}%</div>
			...
			<div
				...
				<button
					...
					{isScreenSharing ? (
						<ScreenShareOff className="w-5 h-5 md:w-6 md:h-6" />
					) : (
